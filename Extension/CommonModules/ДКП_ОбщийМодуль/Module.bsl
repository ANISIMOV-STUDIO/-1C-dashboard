...
Функция ПолучитьДанныеKPI(ИмяМакета, ПараметрыОтчета, ПоляИтогов)
	...
	// Поиск строки итогов
	// Предполагаем, что это первая строка с ПУСТЫМИ измерениями. 
	// Для этого нужно знать ключевое измерение в запросе СКД.
	// Допустим, для всех СКД это поле "Организация".
	Для Каждого Стр Из Таблица Цикл
        // НЕПРАВИЛЬНО: Если Стр.ЯвляетсяИтогами() Тогда
		// ПРАВИЛЬНО: Проверяем, что ключевая группировка пуста
		Если НЕ ЗначениеЗаполнено(Стр.Организация) Тогда // Пример для группировки "Организация"
			Для Каждого Поле Из ПоляИтогов Цикл
				Если Таблица.Колонки.Найти(Поле) <> Неопределено Тогда // Проверяем наличие колонки
					СтруктураИтогов[Поле] = Стр[Поле];
				КонецЕсли;
			КонецЦикла;
			Прервать; // Итоги найдены, выходим из цикла
		КонецЕсли;
	КонецЦикла;
